...
wire [16:0] ID_EX_EX            = ID_EX[`rID_EX_EX];
...
mux4 muxOrigA  (.entr0(ID_EX_Dado1),   .entr1(32'b0),                     .sel(ID_EX_OrigAULA),   .saida(AULA));
mux4 muxOrigB  (.entr0(ID_EX_Dado2),   .entr1(Imm),                       .sel(ID_EX_OrigBULA),   .saida(BULA));
mux4 muxOrigReg(
...
ramD MemD (.address(EX_MEM_ResULA[11:2]), .clock(clockMem), .data(DadoEscrMem), .wren(EX_MEM_MEM[1]), .q(MemLeitura));
